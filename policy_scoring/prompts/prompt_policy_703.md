...
Policy ID: policy/703  
Policy Name: MiEJscreen  
Type: Analysis and Tools  
Description:  
MiEJscreen is an interactive mapping tool that identifies Michigan communities that may be disproportionately impacted by environmental hazards. The map allows users to explore the environmental, health, and socioeconomic conditions within a specific community, region, or across the entire state. First draft released in April 2022.
...
Case 2:
Policy ID: policy/444
Name: Disadvantaged Communities Definition
Similarity: 0.3438
Score: 1
Type: Analysis and Tools
Description: Defines 'disadvantaged communities'

Explanation: Legal Language: The policy uses descriptive language ("defines") without mandatory terms (e.g., "shall," "must") or voluntary incentives. It functions as a conceptual framework, not a directive.
Source of Authority: Likely established as an administrative or regulatory definition (e.g., under state environmental justice laws), but the definition itself lacks independent legal authority to compel action.
Enforcement Mechanism: No enforcement tools (e.g., penalties, compliance requirements) are tied to the definition. Its purpose is informational, not regulatory.
Obligations: No legal obligations are imposed by the definition alone. Compliance depends on how the definition is applied in other policies (e.g., grant eligibility, regulatory thresholds).
Optionality/Incentives: The definition does not inherently incentivize or penalize entities. Its impact is contingent on integration into binding programs or laws.
