...
Policy ID: policy/26  
Policy Name: Sustainable Communities Strategy GHG Reduction Targets  
Type: Targets/Goal  
Description:  
Sets targets for per capita passenger vehicle GHG emission reductions that Metropolitan planning organization (MPO) Sustainable Community Strategies (SCS) must realize. MPOs must prepare a SCS that will reduce GHG emissions to achieve these regional targets, if feasible to do so. Targets below expressed as ranges based on how CARB categorizes MPOs in the state, but website link provides MPO-specific targets.
